...
function language(spec: Spec, index: number): Question | null {
  ...
  if (spec.itemType === 'Sentence assembly') return { ...common, prompt: '★に入るものはどれか。文全体を組み立てて選びなさい。', tokens: [`${tag}：${person}の発言は ＿＿＿ ＿★＿ ＿＿＿ ＿＿＿ と言える。`], options: [pattern[2], '現状を変える', '契機に', '十分なものだ'], answer: 0, note: `The starred chunk practises ${pattern[0]}; assemble every chunk before choosing.` };
